...
//!
//! Supports Qwen-Max, Qwen-Plus, Qwen-Turbo, and Qwen2 models.
...
use futures::Stream;
use openclaw_core::types::{ContentBlock, Message, MessageContent, Role, TokenUsage, ToolDefinition};
use reqwest::Client;
...
impl QwenProvider {
    ...
    /// Set the model (qwen-max, qwen-plus, qwen-turbo, qwen2.5-72b-instruct, etc.).
...
impl ModelProvider for QwenProvider {
    ...
    async fn complete(
        ...
        debug!("Sending request to Qwen/DashScope API");
...
mod tests {
    ...
    fn test_various_models() {
        ...
        for model in models {
            let provider = QwenProvider::new("key").with_model(model);
