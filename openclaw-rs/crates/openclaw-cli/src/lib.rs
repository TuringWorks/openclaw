...
mod tests {
    ...
    fn test_parse_version() {
        let cli = Cli::try_parse_from(["openclaw", "version"]).unwrap();
        assert!(matches!(cli.command, Commands::Version));
    }

    #[test]
    fn test_parse_config_show() {
        let cli = Cli::try_parse_from(["openclaw", "config", "show"]).unwrap();
        match cli.command {
    ...
    fn test_parse_config_get() {
        let cli = Cli::try_parse_from(["openclaw", "config", "get", "gateway.port"]).unwrap();
        match cli.command {
    ...
    fn test_parse_config_set() {
        let cli =
            Cli::try_parse_from(["openclaw", "config", "set", "gateway.port", "9090"]).unwrap();
        match cli.command {
            Commands::Config(args) => match args.command {
    ...
    fn test_parse_agent_chat() {
        let cli =
            Cli::try_parse_from(["openclaw", "agent", "chat", "--agent", "mybot"]).unwrap();
        match cli.command {
            Commands::Agent(args) => match args.command {
    ...
    fn test_parse_channels_enable() {
        let cli =
            Cli::try_parse_from(["openclaw", "channels", "enable", "telegram"]).unwrap();
        match cli.command {
    ...
    fn test_parse_gateway_run() {
        let cli = Cli::try_parse_from(["openclaw", "gateway", "run"]).unwrap();
        match cli.command {
    ...
    fn test_parse_doctor_full() {
        let cli = Cli::try_parse_from(["openclaw", "doctor", "--full"]).unwrap();
        match cli.command {
    ...
    fn test_parse_init_force() {
        let cli = Cli::try_parse_from(["openclaw", "init", "--force"]).unwrap();
        match cli.command {
            Commands::Init { force } => {
                assert!(force);
            }
