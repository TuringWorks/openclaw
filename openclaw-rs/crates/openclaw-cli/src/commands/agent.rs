    ...
    match args.command {
        AgentCommand::List => {
            ...
            if config.agents.agents.is_empty() {
                ...
                println!("  Run `openclaw agent create <id>` to add one.");
            } else {
        ...
        AgentCommand::Message { agent, message } => {
            ...
            match client.post(&url).json(&payload).send().await {
                ...
                Err(e) => {
                    if e.is_connect() {
                        anyhow::bail!(
                            "Gateway not running on port {}. Start it with `openclaw gateway run`.",
                            port
                        );
                    }
...
mod tests {
    use openclaw_core::config::Config;
    use openclaw_core::types::{AgentConfig, AgentId};
