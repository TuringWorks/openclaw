...
//!
//! Provides `openclaw agent chat` -- an interactive session with
//! rustyline line editing, streaming responses, and markdown rendering.

use crate::render;
use openclaw_agent::providers::StreamEvent;
...
impl Default for ReplConfig {
    fn default() -> Self {
        let history_file = openclaw_core::paths::base_dir()
            .map(|d| d.join("history"))
            .unwrap_or_else(|_| PathBuf::from(".openclaw_history"));
