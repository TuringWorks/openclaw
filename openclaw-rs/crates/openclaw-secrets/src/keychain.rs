...
//! 1. `OPENCLAW_MASTER_KEY` environment variable (hex-encoded)
...
const ENV_VAR: &str = "OPENCLAW_MASTER_KEY";

/// Retrieve the master key, creating one if it does not exist yet.
///
/// Resolution order:
/// 1. `OPENCLAW_MASTER_KEY` env var (hex-encoded 32 bytes)
...
fn get_from_keychain() -> Result<Option<Vec<u8>>> {
    ...
    // For now only the OPENCLAW_MASTER_KEY env var is supported on Linux.
