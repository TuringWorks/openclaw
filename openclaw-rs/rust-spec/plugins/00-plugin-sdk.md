...
The Plugin SDK allows extending OpenClaw with custom functionality including tools, hooks, channels, providers, and HTTP routes.
...
/// Plugin manifest (from openclaw.plugin.json).
...
impl PluginRuntime {
    /// Load configuration.
    pub async fn load_config(&self) -> Result<OpenClawConfig, PluginError> {
        self.config_loader.load().await
    }

    /// Write configuration.
    pub async fn write_config(&self, config: &OpenClawConfig) -> Result<(), PluginError> {
...
impl PluginLoader {
    ...
    pub async fn load(
        ...
        // Load manifest
        let manifest_path = source.join("openclaw.plugin.json");
